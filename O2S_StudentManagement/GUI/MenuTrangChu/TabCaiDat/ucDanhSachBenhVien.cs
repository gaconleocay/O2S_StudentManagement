...
namespace O2S_StudentManagement.GUI.MenuTrangChu
{
    public partial class ucDanhSachBenhVien : UserControl
    {
        ...
        private void LoadDanhSachBenhVien()
        {
            try
            {
                string sqldsbv = "select row_number () over (order by benhvienkcbbd) as stt, benhvienid, benhvienkcbbd, benhviencode, benhvienname, benhvienaddress, benhvienhang, benhvientuyen, ghichu from cp_benhvien;";
                DataView dataOption = new DataView(condb.GetDataTable(sqldsbv));
        ...
        private void btnOptionOK_Click(object sender, EventArgs e)
        {
            try
            {
                if (benhvienid != "")
                {
                    string sqlupdate = "UPDATE cp_benhvien SET benhvienname='" + txtbenhvienname.Text.Trim() + "', benhviencode='" + txtbenhviencode.Text.Trim() + "', benhvienaddress='" + txtbenhvienaddress.Text.Trim() + "' WHERE benhvienid='" + benhvienid + "'; ";
                ...
                {
                    string sqlupdate = "INSERT INTO cp_benhvien(benhvienkcbbd, benhviencode, benhvienname, benhvienaddress) VALUES ('" + txtbenhvienkcbbd.Text.Trim() + "', '" + txtbenhviencode.Text.Trim() + "', '" + txtbenhvienname.Text.Trim() + "', '" + txtbenhvienaddress.Text.Trim() + "');";
                    if (condb.ExecuteNonQuery(sqlupdate))
                    {
