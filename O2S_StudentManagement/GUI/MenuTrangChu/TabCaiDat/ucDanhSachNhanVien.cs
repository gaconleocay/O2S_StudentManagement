    ...
    public partial class ucDanhSachNhanVien : UserControl
    {
        ...
        private void ucDanhSachNhanVien_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                DataView dv = new DataView(condb.GetDataTable(sqldsnv)); //Lay danh sach tu ben HIS
                if (dv.Count > 0)
                {
        ...
        private void btnNVOK_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (condb.ExecuteNonQuery(sql))
                    {
                        O2S_StudentManagement.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_StudentManagement.Utilities.ThongBao.frmThongBao(O2S_StudentManagement.Base.ThongBaoLable.SUA_THANH_CONG);
                        frmthongbao.Show();
                    }
                    gridControlDSNV.DataSource = null;
                    ucDanhSachNhanVien_Load(null, null);
        ...
        private void btnThemTuExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (openFileDialogSelect.ShowDialog() == DialogResult.OK)
                {
                    ...
                    O2S_StudentManagement.Base.ReadExcelFile _excel = new O2S_StudentManagement.Base.ReadExcelFile(openFileDialogSelect.FileName);
        ...
        void itemXoaNguoiDung_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    ...
                    O2S_StudentManagement.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_StudentManagement.Utilities.ThongBao.frmThongBao("Đã xóa bỏ tài khoản: " + usercode);
                    frmthongbao.Show();
                    gridControlDSNV.DataSource = null;
                    ucDanhSachNhanVien_Load(null, null);
                }
