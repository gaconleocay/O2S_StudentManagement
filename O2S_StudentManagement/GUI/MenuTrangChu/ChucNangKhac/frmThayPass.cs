        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string sqlquerry = "select * from SM_TBLUSER where usercode='" + en_txtUserID + "' and userpassword='" + en_txtUserPasswordOld + "'";
                    DataView dataBC = new DataView(condb.GetDataTable(sqlquerry));

                    if (dataBC.Count > 0 && txtPasswordNew1.Text == txtPasswordNew2.Text)
                    {
                        string sqlupdate_user = "UPDATE SM_TBLUSER SET userpassword='" + en_txtUserPasswordNew + "' WHERE usercode='" + en_txtUserID + "';";
