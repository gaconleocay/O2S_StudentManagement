...
{
    public partial class ucQLHVDM_Tinh : UserControl
    {
        private DAL.ConnectDatabase condb = new DAL.ConnectDatabase();
        private List<DAL.DM_TINH> lstDichVuImport { get; set; }
        public ucQLHVDM_Tinh()
        {
            InitializeComponent();
        }

        #region Load
        private void ucQLHVDM_Tinh_Load(object sender, EventArgs e)
        {
            try
            {
                LoadDanhMucDichVu();
        ...
        private void LoadDanhMucDichVu()
        {
            ...
            try
            {
                ...
                this.lstDichVuImport = new List<DAL.DM_TINH>();
                if (dataDMDV != null && dataDMDV.Rows.Count > 0)
                {
                    //this.lstDichVuImport = Common.DataTables.ConvertDataTable.DataTableToList<DAL.DM_TINH>(dataDMDV);
                    foreach (DataRow row in dataDMDV.Rows)
                    {
                        //DAL.DM_TINH dichVu = new DAL.DM_TINH();
            ...
            try
            {
                ...
                foreach (var item_dv in this.lstDichVuImport)
                {
                    try
                    {
                        ...
                        //DataTable _dataKiemTra = condb.GetDataTable(_sqlkiemtratrung);
                        ...
                        //    condb.ExecuteNonQuery(_sql_delete);
                        //}
                        ...
                        //if (condb.ExecuteNonQuery(sql_insert))
                        //{
                        //    insert_count += 1;
                        //}
                    }
