...
{
    public partial class frmDanhMucLopHoc_Option1 : Form
    {
        #region Khai bao
        ...
        #endregion
        public frmDanhMucLopHoc_Option1()
        {
            InitializeComponent();
        }
        ...
        private void LoadDotHocTheoLopHoc(int _lophocId)
        {
            try
            {
                List<DOTHOC> _lstDotHoc = DotHocLogic.SelectTheoLopHoc(_lophocId);
                gridControlDSDotHoc.DataSource = _lstDotHoc;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (gridViewLopHoc.RowCount > 0)
                {
                    ...
                    if (this.LopHocSelect != null)
                    {
                        ...
                        LoadDotHocTheoLopHoc(this.LopHocSelect.LopHocId);
                        LockAndUnLookPanelControl(false);
                    }
                }
            }
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Bạn có muốn xóa?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    var rowHandle = gridViewLopHoc.FocusedRowHandle;
                    int _lopHocId = O2S_Common.TypeConvert.Parse.ToInt32(gridViewLopHoc.GetRowCellValue(rowHandle, "LopHocId").ToString());
                    if (LopHocLogic.Delete(_lopHocId))
                    {
        ...
        private void repositoryItemButton_Them_Click(object sender, EventArgs e)
        {
            try
            {
                var rowHandle = gridViewLopHoc.FocusedRowHandle;
                int _LopHocId = O2S_Common.TypeConvert.Parse.ToInt32(gridViewLopHoc.GetRowCellValue(rowHandle, "LopHocId").ToString());
                frmPopUpThemDotHoc _frmDot = new frmPopUpThemDotHoc(_LopHocId);
                _frmDot.ShowDialog();
                gridViewLopHoc_Click(null, null);
            }
            catch (Exception ex)
