...
using O2S_StudentManagement.Common;
...
namespace O2S_StudentManagement.Utilities.PrintPreview
{
    ...
    internal partial class CoolPrintPreviewDialog : Form
    {
        ...
        private void _Zoom_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                double str = O2S_StudentManagement.Common.TypeConvert.TypeConvertParse.ToDouble(_Zoom.EditValue.ToString());
                _preview.Zoom = str;
                _ZoomLable.Caption = (str * 100).ToString() + "%";
            }
            catch (Exception ex)
            {
                O2S_StudentManagement.Common.Logging.LogSystem.Warn(ex);
            }
        }
