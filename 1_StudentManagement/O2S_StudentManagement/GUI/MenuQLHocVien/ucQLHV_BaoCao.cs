...
using O2S_StudentManagement.BUS;
using O2S_StudentManagement.Model.Models;
using O2S_StudentManagement.GUI.FormCommon;

namespace O2S_StudentManagement.GUI.MenuQLHocVien
{
    public partial class ucQLHV_BaoCao : UserControl
    {
        #region Declaration
        ...
        #endregion
        public ucQLHV_BaoCao()
        {
            InitializeComponent();
        }
        public ucQLHV_BaoCao(bool _hienthi_btnTroVe)
        {
            InitializeComponent();
        }
        ...
        private void DanhMucTraCuu_LoadDanhSach()
        {
            try
            {
                List<classPermission> lstBaoCao_DanhMucTraCuu = Base.SessionLogin.SessionLstPhanQuyenNguoiDung.Where(o=>o.permissiontype==3 && o.tabMenuId==5).OrderBy(u=>u.permissioncode).ToList();
