...
namespace O2S_CarParking.GUI.MenuTrangChu
{
    ...
    {
        O2S_CarParking.DAL.ConnectDatabase condb = new DAL.ConnectDatabase();
        ...
        {
            SplashScreenManager.ShowForm(typeof(O2S_CarParking.Utilities.ThongBao.WaitForm1));
            try
            {
...
parentForRootNodes, null);
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    O2S_CarParking.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_CarParking.Utilities.ThongBao.frmThongBao(O2S_CarParking.Base.ThongBaoLable.CHUA_CHON_TEMPLATE);
                    frmthongbao.Show();
                }
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                else if (tenbaocao == "Cập nhật template kết quả")
                {
                    if (openFileDialogImport.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        {
                            O2S_CarParking.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_CarParking.Utilities.ThongBao.frmThongBao(Base.ThongBaoLable.KHONG_TIM_THAY_BAN_GHI_NAO);
                            frmthongbao.Show();
                            return;
                        }
                    }
